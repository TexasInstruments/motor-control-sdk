...
TI HDSL Solution's register map is compatible with SICK HDSL IP Core release version 1.07, with a few exceptions marked with "Not available in TI HDSL Solution", or "Different implementation from SICK HDSL IP Core", or "Register address is different from SICK HDSL IP Core" in the table below.
...
<tr>
    ...
    <td> PRST: Protocol reset
         ...
         - 1 = A forced reset of the protocol status will be initiated. If the bit is cleared, a restart of the connection is triggered.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> LINK: DSL protocol connection status
         ...
         - 1 = Protocol connection between DSL receiver and encoder was established.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Events (High Byte)
         ...
         - All messaging bits are set by the HDSL receiver if a corresponding status is determined.
         - An event bit that has been set is not reset by the HDSL receiver.
...
<tr>
    ...
    <td> SUM: Remote event monitoring
         - 0 = All HDSL encoder events are cleared.
         - 1 = The HDSL encoder has signaled an event and the summary mask is set accordingly (see registers MASK_SUM and SUMMARY).

         When the SUM bit is set, an error or a warning has been transmitted from the HDSL encoder. The application must check the SUMMARY register to obtain a detailed description.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> MIN: Message initialization
         ...
         - 1 = An acknowledgment was received from the HDSL encoder for the initialization of a message.
...
<tr>
    ...
    <td> ANS: Erroneous answer to "long message"
         ...
         This error indicates that the transmission of an answer from the HDSL encoder to the last "long message" failed. The application must send the "long message" again.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> MSUM: Mask for remote event monitoring
         - 0 = HDSL encoder events that are masked in the SUMMARY register do not set the event interrupt.
         - 1 = HDSL encoder events that are masked in the SUMMARY register set the event interrupt.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> MMIN: Mask for message initialization confirmation
         - 0 = The acknowledgment for the initialization of a HDSL encoder message does not set the event interrupt.
         - 1 = The acknowledgment for the initialization of a HDSL encoder message sets the event interrupt.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td>
    <td> Summary mask
         - In this register, the HDSL encoder collective events are determined with which the SUM event monitoring in the event register as well as the signal to the interrupt pin are set (interrupt).
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td>
    <td> Version
         <br/> **NOTE: Different implementation from SICK HDSL IP Core**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Encoder ID (Byte 2) <br/>
         The ENC_ID registers (ENC_ID2, ENC_ID1 and ENC_ID0) contain the designation code of the motor feedback system connected to the HDSL receiver. In the current protocol specification, the designation code is 20 bits long.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Position checksum, Channel 1 (High Byte)
         ...
         - The CRC is checked in the HDSL receiver IP Core.
...
<tr>
    ...
    <td> Parameters Channel Control
         - This register for the Parameters Channel handles the start of "long message" transactions. After setting all "long message" registers (registers PC_BUFFER0 to 7, PC_ADD_H/PC_ADD_L and PC_OFF_H/L), the "long message" is transmitted to the HDSL encoder by setting the LSTA bit.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td>
    <td> Safe Summary
         - This register contains the summarized HDSL encoder status information for the safety related application.
         - It is based on the encoder status ENC_ST7:0.
         - Each status summary bit contains the summarized information from 8 error, warning and event modes of the HDSL encoder.
</tr>
<tr>
    <td>
    <td>
    ...
    <td> SSUM7:SSUM1: Status Summary bit (external resource)
         ...
         - 1 = An error, a warning or an event associated with HDSL encoder external resources was triggered.
</tr>
<tr>
    <td>
    <td>
    ...
    <td> SSUM0: Status summary bit (interface)
         - 0 = The HDSL encoder protocol has not triggered an error, a warning or event.
         - 1 = An error, a warning or an event associated with the HDSL encoder protocol interface was triggered.
</tr>
<tr>
    <td> S_PC_DATA
    <td> 0x37
    ...
    <td> "Short message" Parameters Channel Data
         ...
         - "Short message" transactions are generated if operations are carried out with remote registers (HDSL encoder).
...
<tr>
    ...
    <td> Safe Events
         ...
         - All messaging bits are set by the HDSL receiver if a corresponding status is determined.
         - An event bit that has been set is not reset by the HDSL receiver.
         - The safety related application must clear bits that have been set.
...
<tr>
    ...
    <td> SSUM: Remote event monitoring
         - 0 = All HDSL encoder events are cleared.
         - 1 = The HDSL encoder has signaled an event.

         When the bit is set, an error or a warning has been transmitted from the HDSL encoder. The safety related application must check the SAFE_SUM register to obtain a detailed description.
<tr>
    <td>
    <td>
    <td> 5
...
<tr>
    ...
    <td> PRST: Protocol reset warning
         ...
         This error message indicates that the protocol connection to the HDSL encoder has been re-initialized. This error message can be caused by a frequency inverter application request (PRST bit in SYS_CTRL), a safety related application request (PRST bit in SAFE_CTRL), or generated by the HDSL receiver itself. The HDSL receiver causes a protocol reset if too many transmission errors indicate a connection problem. A protocol reset causes a re-synchronization with the HDSL encoder that can improve the connection quality.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> FRES: Channel free for "short message"
         ...
         If the bit is set, the frequency inverter application can trigger a "short message". Provided no answer has been received from the HDSL encoder, this bit remains cleared. As the processing duration of a "short message" in the motor feedback system is not specified, a time limit condition is installed in the HDSL receiver. If the time limit is exceeded, attempts are made again automatically.
</tr>
<tr>
    <td> MASK_S
    <td> 0x3E
    ...
    <td> Safe Event Mask
         ...
         - Several events can be masked to trigger a safe event interrupt.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> MSSUM: Mask for remote event monitoring
         - 0 = HDSL encoder events that are set in the SAFE_SUM register do not set the safe event interrupt.
         - 1 = HDSL encoder events that are set in the SAFE_SUM register set the safe event interrupt.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> 1
    <td> MMIN: Mask for message initialization confirmation
         - 0 = The acknowledgment for the initialization of a HDSL encoder message does not set the safe event interrupt.
         - 1 = The acknowledgment for the initialization of a HDSL encoder message sets the safe event interrupt.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Version in Safe Channel 2 (Identical to VERSION register)
         <br/> **NOTE: Different implementation from SICK HDSL IP Core**
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x44 instead of 0x0B)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Encoder ID in Safe Channel 2
         <br/> **NOTE: Not available in TI HDSL Solution**
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x45 instead of 0x0F)**
</tr>
<tr>
    <td> STATUS2
    <td> 0x46
    ...
    <td> Safe Channel 2 Status
         ...
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x46 instead of 0x18)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Safe Position, Channel 2 (Byte 4)
         ...
         - The units of the position value are (steps).
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x47 instead of 0x19)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Safe Position, Channel 2 (Byte 3)
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x48 instead of 0x1A)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Safe Position, Channel 2 (Byte 2)
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x49 instead of 0x1B)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Safe Position, Channel 2 (Byte 1)
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x4A instead of 0x1C)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Safe Position, Channel 2 (Byte 0)
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x4B instead of 0x1D)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Position checksum, Channel 2 (High Byte)
         ...
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x4C instead of 0x1E)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Position checksum, Channel 2 (Low Byte)
         <br/> **NOTE: Register address is different from SICK HDSL IP Core (TI implementation uses 0x4D instead of 0x1F)**
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Online Status D (High Byte)
         - The Online Status D is a non-storing copy of registers EVENT_H and EVENT_L. The static information in these event registers must be cleared by the user after the read process, by writing the value "0" to the corresponding bit in the register, whilst the Online Status D only shows the current status without storing previous indications.
</tr>
<tr>
    <td>
    <td>
...
<tr>
    ...
    <td> Online Status 1 (High Byte)
         - The Online Status 1 is a non-storing copy of registers EVENT_S. The static information in the event register must be cleared by the user after the read process, by writing the value "0" to the corresponding bit in the register, whilst the Online Status 1 only shows the current status without storing previous indications.
...
<tr>
    ...
    <td>
    <td> 7:3
    <td> FIX0: These bits always give a "0".
         <br/> **NOTE: Different implementation from SICK HDSL IP Core**
</tr>
<tr>
    <td>
