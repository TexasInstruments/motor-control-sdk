...
#### IEP CMP Event Selection{#IEP_CMP_EVENT_SELECTION}
The Industrial Ethernet Peripheral (IEP) contains 16 compare events (CMP0-CMP15) that can be used to trigger %SDFM normal current sampling. With SysConfig support, users can now select which IEP CMP events to use for triggering %SDFM sampling based on their system requirements.

##### CMP Event Usage and Reservation
- **CMP0**: Reserved for IEP counter reset (EPWM synchronization). Never use for sampling trigger.
- **CMP1-CMP2**: May conflict with IEP SYNC0/SYNC1 output generation when using IEP for clock generation.
- **CMP3-CMP6, CMP9-CMP15**: Available for user configuration.
- **CMP4**: Default and recommended for single PRU mode or PRU core in load-share mode.
- **CMP7**: Default for RTU-PRU core in load-share mode.
- **CMP8**: Default for TX-PRU core in load-share mode.

For detailed CMP event configuration guidance, refer to \ref SDFM_IEP_CMP_CONFIG.
