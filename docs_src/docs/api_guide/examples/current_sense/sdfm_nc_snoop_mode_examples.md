# %SDFM Examples with snoop mode based normal current sampling  {#BASIC_SDFM_EXAMPLES_WITH_SNOOP_MODE_NC}

[TOC]
This example uses ICSS %SDFM snoop mode for sampling, which provides flexibility to choose different OSR values between Normal current and Over current.
These examples demonstrate both trigger-based and continuous normal current sampling. The examples support Normal current Over-sampling Ratio (OSR), Over current OSR, and Normal current trigger time configuration.

There are two examples based on the number of %SDFM channels.

\note These examples use the IEP CMP event for %SDFM snoop mode. However, these examples may not be reliable in certain situations:
...
<tr>
        <td>3. To check Normal Current (NC) Samples with Different NC OSR Values</td>
...
<tr>
        <td>5. To check NC samples with different sdfm clock values</td>
        <td>1. Set NC OSR to 64</td>
...
<tr>
        <td>6. To test SINC1/SINC2/SINC3 filter</td>
        <td>1. Set NC OSR to 64 </td>
...
<tr>
        <td></td>
        <td>2. \image html SDFM_EPWM1_HW_Setup.png "SDFM clock from EPWM"</td>
        <td></td>
 </tr>
 <tr>
        <td></td>
