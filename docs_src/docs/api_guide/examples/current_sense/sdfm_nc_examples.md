...
\note Normal Current trigger mode examples do not support Over Current. 
...
1. **Continuous Mode Example**
        ...
        - **Note:** **Host Interrupt Limitation for Nine-Channel Continuous Mode:** PRU-ICSS has only 8 host interrupts (HOST_INTR_PEND_0-7), but SDFM supports 9 channels. By default, channel 8 uses HOST_INTR_PEND_0, which conflicts with channel 0 when both are enabled in continuous mode with individual IRQ handlers. For nine-channel continuous mode:
                - **Recommended:** Use common IRQ mode (as implemented in the load-share example) to avoid the conflict
                - **Alternative:** Reconfigure channel 8 to use an unused host interrupt via SysConfig INTC mapping (e.g., if channels 3-5 are unused, their host interrupts can be reassigned to channel 8)
...
\note **IRQ Configuration Options:** The SDK provides two interrupt handler configurations for continuous mode examples:
        - **Common IRQ (Load Share Mode):** The nine-channel load share example uses common IRQ mode where each PRU core has one shared interrupt handler for its three channels. This reduces interrupt overhead and is recommended for multi-channel applications.
                **Note:** It is not implemented for single PRU (non load share) continuous mode, user can implement a common IRQ handler for all connected channels in single PRU in the same way as load share mode.
