...
//FIXME: Add FOC timing with EtherCAT sync, EPWM TB and trigger points for encoder and %SDFM

### Components

#### EnDat Configuration
The EnDat interface is used for position feedback. It operates in periodic mode and is synchronized with the EPWM SYNC out event. For Motor 1, Channel 0 of PRU slice 1 runs on the RTU core. For Motor 2, Channel 2 of PRU slice 1 runs on the TX PRU core. Both PRUs send commands to the encoder and store the position value in memory, which is used by R5 for position feedback.
...
\note EPWM tripe zone is not enabled.
...
The project has 2 sub-projects:
 - `C:\ti\motor_control_sdk_am243x_xx\examples\tidep_01032_dual_motor_drive\universal_single_chip_servo\am243x-lp-BP-AM2BLDCSERVO\r5fss0-0_nortos` (universal_single_chip_servo_am243x-lp_r5fss0-0_nortos_ti-arm-clang_BP_AM2BLDCSERVO: motor drive project for both axes)
 - `C:\ti\motor_control_sdk_am243x_xx\examples\tidep_01032_dual_motor_drive\universal_single_chip_servo\am243x-lp-BP-AM2BLDCSERVO\r5fss1-0_freertos` (ethercat_subdevice_cia402_demo_am243x-lp_r5fss1-0_freertos_ti-arm-clang: EtherCAT CiA402 client code)
...
8. To use DATALOG with the graph tool:
   - To test the phase currents using the DATALOG module, the following code must be set up in the
sys_main.c file. 
  ```c
  // Note this code is already configured by default for Motor 1 in `sys_main.c`(similar configuration is possible for motor2):
  ...
  datalogObj->iptr[3] = (float32_t*) &motorVars_M1.sdfmData.I_A.value[3];
  datalogObj->iptr[3] = (float32_t*) &motorVars_M1.angleFOC_rad;
  ```
   - Enable DATALOG in predefined symbols
   - Rebuild the example and import data log file in Graph tool window form `<workspace>\universal_motorcontrol_am243x_r5fss0-0_nortos_ti-armclang\src_control\debug\` 
...
- Set motorVars_M1.flagEnableRunAndIdentify to 0 to stop the motor
\note The steps for Motor 2 are the same as for Motor 1. Just update and observe the variables of motorVars_M2.
...
##### Run and Results 
1. Load and Run R5F_0_0 example
2. Halt R5F_0_0 and import universal_motor1_control_level4.txt and universal_motor2_control_level4.txt into the Expressions window
...
7. To verify the encoder measured angle `angleENC_rad`, enable and configure the DATALOG module as described in Build level2. 
...
2. Set `flagEnableRunAndIdentify` to 1 for both motor
3. Control motor speed through the EtherCAT CiA402 Client Example section instructions

#### EtherCAT CiA402 Client Example 
//FIXME Add all TWinCAT setup images

Section describes the steps to run the EtherCAT sub-device CiA402 demo example (R5F_1_0) and control the speed of both motors using TwinCAT.
- Connect the R5F_1_0
...
- Create an EtherCAT Project in TwinCAT
- Scan for devices by right-clicking on Devices → Scan
- Verify Device 1 (TI EtherCAT Toolkit CiA402 for AM243X.R5F) is found

##### Motor speed Control

- Set RxPDO (Motor 1) Target velocity to `240` (240 RPM)
- Set RxPDO (Motor 1) Modes of Operation to `"9"` (Cyclic synchronous velocity mode)
- Set RxPDO (Motor 1) Controlword to `"15"` (Switch On | Enable Voltage | Quick Stop | Enable Operation)
- Verify Motor 1 speed changes from `120` RPM to `240` RPM
- Verify TxPDO (Motor 1) Velocity actual value shows `240` RPM

- Set RxPDO_1 (Motor 2) Target velocity to `180` (180 RPM)
- Set RxPDO_1 (Motor 2) Modes of Operation to `"9"`
- Set RxPDO_1 (Motor 2) Controlword to `"15"`
- Verify Motor 2 reaches target speed of `180` RPM
- Verify TxPDO1 (Motor 2) Velocity actual value shows `180` RPM
